// voxel-logarithmic-conch: independent runtime shader; mathematical occupancy rendered with 3D DDA.
...
// Logarithmic Conch — a hollow tube swept along r=.58 exp(.19 theta).
// Reference for the generating spiral: https://mathworld.wolfram.com/LogarithmicSpiral.html
// Artistic shell cross-section/ribs are additions, not a measured biological model.
static float2 voxelField(float3 p,float time){
 if(length(p)>.83f)return float2(0);
 float radius=length(p.xy),angle=atan2(p.y,p.x);
 float best=2.0f,material=0;
 for(int turn=-3;turn<=0;turn++){
  float theta=angle+float(turn)*6.2831853f;
  if(theta>0.0f||theta< -17.0f)continue;
  float spiral=.58f*exp(.19f*theta);
  float tube=.23f*spiral+.019f;
  float rib=.004f*cos(theta*38.0f);
  float wall=abs(length(float2(radius-spiral,p.z))-(tube+rib))-.012f;
  if(wall<best){best=wall;material=.5f+.5f*cos(theta*22.0f);}
 }
 return float2(1.0f-smoothstep(0.0f,.012f,best),material);
}
    ...
    for (int step=0; step<224 && travel<span.y; ++step) {
        ...
        if (sample.x>0.025f) {
            ...
            if (voxelBox(ro,rd,center,halfSize,hitT,n) && hitT>=travel-0.0001f && hitT<=exitT+0.0001f) {
                ...
                float vein=sample.y;
                float3 base=mix(float3(.025f,.25f,.31f),float3(.25f,.68f,.68f),vein);
                float gold=smoothstep(.72f,.98f,sample.y);
                base=mix(base,float3(.93f,.79f,.53f),gold);
